...
public class FPSCamera extends Parent {
		...
		scene.addEventHandler(MouseEvent.ANY, me -> {
			if (me.getEventType().equals(MouseEvent.MOUSE_PRESSED)) {
				...
				if (me.isPrimaryButtonDown()) {
					...
					rotateY.setAngle(MathUtils.clamp(-360,
							((rotateY.getAngle() + mouseDeltaX * (mouseSpeed * mouseModifier)) % 360 + 540) % 360 - 180,
							360)); // horizontal
					rotateX.setAngle(MathUtils.clamp(-180,
							((rotateX.getAngle() - mouseDeltaY * (mouseSpeed * mouseModifier)) % 360 + 540) % 360 - 180,
							35)); // vertical
	...
	/*
	 * ==========================================================================
	 * Callbacks | R | Up| F | | P| U |mxx|mxy|mxz| |tx| V |myx|myy|myz| |ty| N
	 * |mzx|mzy|mzz| |tz|
	 * 
	 */
